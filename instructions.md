...
This scraper extracts job listings from ARRISE careers page (Romania only) and imports them to peviitor.ro.

Target: https://careers.arrise.com
...
1. **Start with brand** - We know the brand (e.g., "ARRISE")
...
8. **Scrape new jobs** - Extract jobs from ARRISE careers page (Romania)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts ARRISE Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from ARRISE careers API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape ARRISE API (jobs for Romania)
    │
    ▼
